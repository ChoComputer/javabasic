...
public class Archer {
	...
	public void huntOrc(Orc orc) {
		System.out.println("오크의 상태" + orc.getHp());
	...
	public void huntDragon(Dragon dragon) {
		// 공격 전에 먼저 드래곤의 체력을 체크해서 죽은 트롤이라면
		// 따로 공격을 할 수 없도록 메서드를 바로 탈출시킵니다.
		if (dragon.getHp() <= 0) {
	...
	public void huntGoul(Goul g) {
		...
		if (g.getHp() <= 0) {
			System.out.println("이미 죽어 있는 구울이군.");
			System.out.println("--------------------------------.");
			return;
		}
		g.dobattle(this.atk);

		this.hp = (this.hp + this.def) - g.getAtk();
		if (this.hp <= 0) {
			System.out.println("나의 죽음을...알려라...");
			return;
		}
		if (g.getHp() <= 0) {
		...
		if (this.hp > 0 && g.getHp() > 0) {
			
			count++;
			System.out.println("Goul과의 " + count + "번 쨰 교전 완료!");
			System.out.println("나의 hp: " + this.hp);
			System.out.println("Goul의 hp: " + g.getHp());
			System.out.println("--------------------------------");
		}
